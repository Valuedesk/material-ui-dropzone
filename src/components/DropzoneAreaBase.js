...
class DropzoneAreaBase extends React.PureComponent {
    ...
    handleDropAccepted = async(acceptedFiles, evt) => {
        const {fileObjects, filesLimit, getFileAddedMessage, getFileErrorMessage, getFileLimitExceedMessage, onAdd, onDrop} = this.props;
        ...
        if (unreadableFiles.length > 0) {
            const message = unreadableFiles.reduce((msg, fileObj) => msg + getFileErrorMessage(fileObj.file, fileObj.error), '');
            this.setState({
                openSnackBar: true,
...
DropzoneAreaBase.defaultProps = {
    ...
    getFileErrorMessage: (file, error) => (`Error opening file ${file.name}`),
